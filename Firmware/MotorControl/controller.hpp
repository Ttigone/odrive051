 ...
 public:
  ...
  struct Config_t {
    ControlMode control_mode =
        CONTROL_MODE_POSITION_CONTROL;              // see: ControlMode_t
    ...
    // float vel_gain = 0.2f / 200.0f,       // [Nm/(rad/s)] <sensorless
    // example>
    ...
    float circular_setpoint_range =
        1.0f;  // Circular range when circular_setpoints is true. [turn]
    ...
    bool enable_current_mode_vel_limit =
        true;  // enable velocity limit in current control mode (requires a
               // valid velocity estimator)
    uint8_t axis_to_mirror = -1;
    float mirror_ratio = 1.0f;
    uint8_t load_encoder_axis = -1;  // default depends on Axis number and is
                                     // set in load_configuration()
  ...
  constexpr void input_pos_updated() { input_pos_updated_ = true; }
  // 编码器选择
