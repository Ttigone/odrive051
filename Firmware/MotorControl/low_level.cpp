...
// Arbitrary non-zero inital value to avoid division by zero if ADC reading is
// late
 ...
 *   - After startup, all variables used in this section are exclusively
 * modified by the code in this section (this excludes function parameters) This
 * assumption also requires that there is no memory corruption.
...
// @brief Floats ALL phases immediately and disarms both motors and the brake
// resistor.
...
// timer period.
void safety_critical_apply_motor_pwm_timings(Motor& motor,
                                             uint16_t timings[3]) {
  uint32_t mask = cpu_enter_critical();
  if (!brake_resistor_armed) {
    motor.armed_state_ = Motor::ARMED_STATE_DISARMED;
...
void safety_critical_apply_brake_resistor_timings(uint32_t low_off,
                                                  uint32_t high_on) {
...
void start_adc_pwm() {
  ...
  sync_timers(&htim1, &htim8, TIM_CLOCKSOURCE_ITR0,
              TIM_1_8_PERIOD_CLOCKS / 2 - 1 * 128, &htim13);
...
// This conversion is triggered by TIM1 at the frequency of the motor control
// loop.
void start_general_purpose_adc() {
  ...
  HAL_ADC_Start_DMA(&hadc1, reinterpret_cast<uint32_t*>(adc_measurements_),
                    ADC_CHANNEL_COUNT);
}
...
uint16_t channel_from_gpio(const GPIO_TypeDef* const GPIO_port,
                           uint16_t GPIO_pin) {
...
float get_adc_voltage_channel(uint16_t channel) {
  if (channel < ADC_CHANNEL_COUNT)
    return ((float)adc_measurements_[channel]) *
           (adc_ref_voltage / adc_full_scale);
  else
...
// an ADC conversion. Timing diagram: Firmware/timing_diagram_v3.png
...
#define calib_tau 0.2f  //@TOTO make more easily configurable
  ...
  // If the corresponding timer is counting up, we just sampled in SVM vector 0,
  // i.e. real current If we are counting down, we just sampled in SVM vector 7,
  // with zero current
  ...
  if (update_timings) {
    if (!other_axis.motor_.next_timings_valid_) {
      ...
      safety_critical_apply_motor_pwm_timings(other_axis.motor_,
                                              other_axis.motor_.next_timings_);
    }
  ...
  if (current_meas_not_DC_CAL) {
    ...
    // before ADC3. Therefore we store the value from ADC2 and signal the thread
    // that the measurement is ready when we receive the ADC3 measurement
    ...
    // DC_CAL measurement
    if (hadc == &hadc2) {
      axis.motor_.DC_calib_.phB +=
          (current - axis.motor_.DC_calib_.phB) * calib_filter_k;
    } else {
      axis.motor_.DC_calib_.phC +=
          (current - axis.motor_.DC_calib_.phC) * calib_filter_k;
    }
  }
}
...
void update_brake_current() {
  ...
  float brake_duty =
      brake_current * odrv.config_.brake_resistance / vbus_voltage;

  if (odrv.config_.enable_dc_bus_overvoltage_ramp &&
      (odrv.config_.brake_resistance > 0.0f) &&
      (odrv.config_.dc_bus_overvoltage_ramp_start <
       odrv.config_.dc_bus_overvoltage_ramp_end)) {
    brake_duty +=
        std::fmax((vbus_voltage - odrv.config_.dc_bus_overvoltage_ramp_start) /
                      (odrv.config_.dc_bus_overvoltage_ramp_end -
                       odrv.config_.dc_bus_overvoltage_ramp_start),
                  0.0f);
  }
  ...
  Ibus_sum += brake_duty
                  ? (brake_duty * vbus_voltage / odrv.config_.brake_resistance)
                  : 0.0f;
...
void pwm_in_init() {
  ...
  {
#endif
    if (fibre::is_endpoint_ref_valid(
            odrv.config_.pwm_mappings[gpio_num - 1].endpoint)) {
      GPIO_InitStruct.Pin = get_gpio_pin_by_pin(gpio_num);
      HAL_GPIO_DeInit(get_gpio_port_by_pin(gpio_num),
                      get_gpio_pin_by_pin(gpio_num));
      HAL_GPIO_Init(get_gpio_port_by_pin(gpio_num), &GPIO_InitStruct);
      HAL_TIM_IC_ConfigChannel(&htim5, &sConfigIC,
                               gpio_num_to_tim_2_5_channel(gpio_num));
      HAL_TIM_IC_Start_IT(&htim5, gpio_num_to_tim_2_5_channel(gpio_num));
    }
  }
}

// TODO: These expressions have integer division by 1MHz, so it will be
// incorrect for clock speeds of not-integer MHz
#define TIM_2_5_CLOCK_HZ TIM_APB1_CLOCK_HZ
#define PWM_MIN_HIGH_TIME           \
  ((TIM_2_5_CLOCK_HZ / 1000000UL) * \
   1000UL)  // 1ms high is considered full reverse
#define PWM_MAX_HIGH_TIME           \
  ((TIM_2_5_CLOCK_HZ / 1000000UL) * \
   2000UL)  // 2ms high is considered full forward
#define PWM_MIN_LEGAL_HIGH_TIME     \
  ((TIM_2_5_CLOCK_HZ / 1000000UL) * \
   500UL)  // ignore high periods shorter than 0.5ms
#define PWM_MAX_LEGAL_HIGH_TIME     \
  ((TIM_2_5_CLOCK_HZ / 1000000UL) * \
   2500UL)  // ignore high periods longer than 2.5ms
#define PWM_INVERT_INPUT false

// 脉冲处理
void handle_pulse(int gpio_num, uint32_t high_time) {
  if (high_time < PWM_MIN_LEGAL_HIGH_TIME ||
      high_time > PWM_MAX_LEGAL_HIGH_TIME) {
    // 小于最小 500us 脉宽, 大于 2500us 不执行
    return;
  }
  ...
  float value = odrv.config_.pwm_mappings[gpio_num - 1].min +
                (fraction * (odrv.config_.pwm_mappings[gpio_num - 1].max -
                             odrv.config_.pwm_mappings[gpio_num - 1].min));

  fibre::set_endpoint_from_float(
      odrv.config_.pwm_mappings[gpio_num - 1].endpoint, value);
}
...
void pwm_in_cb(int channel, uint32_t timestamp) {
  ...
  if (gpio_num < 1 || gpio_num > GPIO_COUNT) return;
  bool current_pin_state =
      HAL_GPIO_ReadPin(get_gpio_port_by_pin(gpio_num),
                       get_gpio_pin_by_pin(gpio_num)) != GPIO_PIN_RESET;
...
static void update_analog_endpoint(const struct PWMMapping_t* map, int gpio) {
  float fraction =
      get_adc_voltage(get_gpio_port_by_pin(gpio), get_gpio_pin_by_pin(gpio)) /
      3.3f;
...
void start_analog_thread() {
  osThreadDef(thread_def, analog_polling_thread, osPriorityLow, 0,
