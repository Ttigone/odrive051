...
class Motor : public ODriveIntf::MotorIntf {
 public:
  ...
  // example: current_lim and calibration_current will instead determine the
  // maximum voltage applied to the motor.
  struct Config_t {
    bool pre_calibrated =
        false;  // can be set to true to indicate that all values here are valid
    int32_t pole_pairs = 7;
    float calibration_current = 10.0f;  // [A]
    float resistance_calib_max_voltage =
        2.0f;  // [V] - You may need to increase this if this voltage isn't
               // sufficient to drive calibration_current through the motor.
    float phase_inductance = 0.0f;  // to be set by measure_phase_inductance
    float phase_resistance = 0.0f;  // to be set by measure_phase_resistance
    float torque_constant =
        0.04f;  // [Nm/A] for PM motors, [Nm/A^2] for induction motors. Equal
                // to 8.27/Kv of the motor
    ...
    void set_pre_calibrated(bool value) {
      ...
      parent->is_calibrated_ =
          parent->is_calibrated_ || parent->config_.pre_calibrated;
    }
  ...
  Motor(const MotorHardwareConfig_t& hw_config,
        const GateDriverHardwareConfig_t& gate_driver_config, Config_t& config);

  bool arm();
  void disarm();
  void setup() { DRV8301_setup(); }
  ...
  uint16_t next_timings_[3] = {TIM_1_8_PERIOD_CLOCKS / 2,
  ...
  float phase_current_rev_gain_ =
      0.0f;  // Reverse gain for ADC to Amps (to be set by DRV8301_setup)
  CurrentControl_t current_control_ = {
      .p_gain = 0.0f,  // [V/A] should be auto set after resistance and
                       // inductance measurement
      .i_gain = 0.0f,  // [V/As] should be auto set after resistance and
                       // inductance measurement
  ...
  DRV_SPI_8301_Vars_t gate_driver_regs_;  // Local view of DRV registers
                                          // (initialized by DRV8301_setup)
