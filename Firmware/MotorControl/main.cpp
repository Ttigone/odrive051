...
typedef Config<
    BoardConfig_t, ODriveCAN::Config_t, Encoder::Config_t[AXIS_COUNT],
    SensorlessEstimator::Config_t[AXIS_COUNT], Controller::Config_t[AXIS_COUNT],
    ...
    TrapezoidalTrajectory::Config_t[AXIS_COUNT], Endstop::Config_t[AXIS_COUNT],
    Endstop::Config_t[AXIS_COUNT], Axis::Config_t[AXIS_COUNT]>
    ConfigFormat;

void ODrive::save_configuration(void) {
  if (ConfigFormat::safe_store_config(
          &odrv.config_, &can_config, &encoder_configs, &sensorless_configs,
          &controller_configs, &motor_configs, &fet_thermistor_configs,
          &motor_thermistor_configs, &trap_configs, &min_endstop_configs,
          &max_endstop_configs, &axis_configs)) {
...
extern "C" int load_configuration(void) {
  ...
  if (NVM_init() ||
    ...
    for (size_t i = 0; i < AXIS_COUNT; ++i) {
      ...
      Axis::load_default_step_dir_pin_config(hw_configs[i].axis_config,
                                             &axis_configs[i]);
...
void ODrive::erase_configuration(void) {
  ...
  // FIXME: this reboot is a workaround because we don't want the next
  // save_configuration to write back the old configuration from RAM to NVM. The
  // proper action would be to reset the values in RAM to default. However right
  // now that's not practical because several startup actions depend on the
  // config. The other problem is that the stack overflows if we reset to
  // default here.
  NVIC_SystemReset();
}
...
void ODrive::enter_dfu_mode() {
  if ((hw_version_major_ == 3) && (hw_version_minor_ >= 5)) {
     ...
     * If you really want to use it on an older board, add 3.3k pull-down
     * resistors to the AUX_L and AUX_H signals and _only then_ uncomment these
     * lines.
     */
    //__asm volatile ("CPSID I\n\t":::"memory"); // disable interrupts
...
extern "C" int construct_objects() {
  ...
  if (odrv.config_.enable_i2c_instead_of_can) {
    ...
    i2c_stats_.addr |=
        HAL_GPIO_ReadPin(I2C_A0_PORT, I2C_A0_PIN) != GPIO_PIN_RESET ? 0x1 : 0;
    i2c_stats_.addr |=
        HAL_GPIO_ReadPin(I2C_A1_PORT, I2C_A1_PIN) != GPIO_PIN_RESET ? 0x2 : 0;
    i2c_stats_.addr |=
        HAL_GPIO_ReadPin(I2C_A2_PORT, I2C_A2_PIN) != GPIO_PIN_RESET ? 0x4 : 0;
  ...
  for (size_t i = 0; i < AXIS_COUNT; ++i) {
    ...
    SensorlessEstimator* sensorless_estimator =
        new SensorlessEstimator(sensorless_configs[i]);
    Controller* controller = new Controller(controller_configs[i]);

    OnboardThermistorCurrentLimiter* fet_thermistor =
        new OnboardThermistorCurrentLimiter(hw_configs[i].thermistor_config,
                                            fet_thermistor_configs[i]);
    OffboardThermistorCurrentLimiter* motor_thermistor =
        new OffboardThermistorCurrentLimiter(motor_thermistor_configs[i]);

    Motor* motor =
        new Motor(hw_configs[i].motor_config, hw_configs[i].gate_driver_config,
                  motor_configs[i]);
    ...
    axes[i] =
        new Axis(i, hw_configs[i].axis_config, axis_configs[i], *encoder,
                 *sensorless_estimator, *controller, *fet_thermistor,
...
void vApplicationIdleHook(void) {
  if (odrv.system_stats_.fully_booted) {
    ...
    odrv.system_stats_.min_stack_space_comms =
        uxTaskGetStackHighWaterMark(comm_thread) * sizeof(StackType_t);
    odrv.system_stats_.min_stack_space_axis0 =
        uxTaskGetStackHighWaterMark(axes[0]->thread_id_) * sizeof(StackType_t);
    odrv.system_stats_.min_stack_space_axis1 =
        uxTaskGetStackHighWaterMark(axes[1]->thread_id_) * sizeof(StackType_t);
    odrv.system_stats_.min_stack_space_usb =
        uxTaskGetStackHighWaterMark(usb_thread) * sizeof(StackType_t);
    odrv.system_stats_.min_stack_space_uart =
        uxTaskGetStackHighWaterMark(uart_thread) * sizeof(StackType_t);
    odrv.system_stats_.min_stack_space_usb_irq =
        uxTaskGetStackHighWaterMark(usb_irq_thread) * sizeof(StackType_t);
    odrv.system_stats_.min_stack_space_startup =
        uxTaskGetStackHighWaterMark(defaultTaskHandle) * sizeof(StackType_t);
    odrv.system_stats_.min_stack_space_can =
        uxTaskGetStackHighWaterMark(odCAN->thread_id_) * sizeof(StackType_t);

    // Actual usage, in bytes, so we don't have to math
    odrv.system_stats_.stack_usage_axis0 =
        axes[0]->stack_size_ - odrv.system_stats_.min_stack_space_axis0;
    odrv.system_stats_.stack_usage_axis1 =
        axes[1]->stack_size_ - odrv.system_stats_.min_stack_space_axis1;
    odrv.system_stats_.stack_usage_comms =
        stack_size_comm_thread - odrv.system_stats_.min_stack_space_comms;
    odrv.system_stats_.stack_usage_usb =
        stack_size_usb_thread - odrv.system_stats_.min_stack_space_usb;
    odrv.system_stats_.stack_usage_uart =
        stack_size_uart_thread - odrv.system_stats_.min_stack_space_uart;
    odrv.system_stats_.stack_usage_usb_irq =
        stack_size_usb_irq_thread - odrv.system_stats_.min_stack_space_usb_irq;
    odrv.system_stats_.stack_usage_startup =
        stack_size_default_task - odrv.system_stats_.min_stack_space_startup;
    odrv.system_stats_.stack_usage_can =
        odCAN->stack_size_ - odrv.system_stats_.min_stack_space_can;
  }
}
}
...
int odrive_main(void) {
  ...
  // Start state machine threads. Each thread will go through various
  // calibration procedures and then run the actual controller loops.
