...
bool SensorlessEstimator::update() {
  ...
  // associated with this cycle is the one computed two cycles ago. To get the
  // correct measurement, it was stored twice: once by
  // final_v_alpha/final_v_beta in the current control reporting, and once by
  // V_alpha_beta_memory.
  ...
  vel_estimate_ =
      vel_estimate_erad_ /
      (std::max((float)axis_->motor_.config_.pole_pairs, 1.0f) * 2.0f * M_PI);
