 ...
 public:
  ...
  struct Config_t {
    ...
    bool startup_encoder_index_search =
        false;  //<! run encoder index search after startup, skip otherwise
                // this only has an effect if encoder.config.use_index is also
                // true
    bool startup_encoder_offset_calibration =
        false;  //<! run encoder offset calibration after startup, skip
                // otherwise
    // 闭环控制
    ...
    bool enable_step_dir =
        false;  //<! enable step/dir input after calibration
                //   For M0 this has no effect if enable_uart is true
    bool step_dir_always_on =
        false;  //<! Keep step/dir enabled while the motor is disabled.
  ...
  enum thread_signals { M_SIGNAL_PH_CURRENT_MEAS = 1u << 0 };

  Axis(int axis_num, const AxisHardwareConfig_t& hw_config, Config_t& config,
       Encoder& encoder, SensorlessEstimator& sensorless_estimator,
       Controller& controller, OnboardThermistorCurrentLimiter& fet_thermistor,
       OffboardThermistorCurrentLimiter& motor_thermistor, Motor& motor,
       TrapezoidalTrajectory& trap, Endstop& min_endstop, Endstop& max_endstop);
  ...
  bool inline check_for_errors() { return error_ == ERROR_NONE; }

  // @brief Runs the specified update handler at the frequency of the current
  // measurements.
  //
  ...
  //
  // If update_handler is going to update the motor timings, you must call
  // motor.arm() shortly before this function.
  //
  // If the function returns, it is guaranteed that error is non-zero, except if
  // the cause for the exit was a negative return value of update_handler or an
  // external state change request (requested_state != AXIS_STATE_DONT_CARE).
  // Under all exit conditions the motor is disarmed and the brake current set
  // to zero. Furthermore, if the update_handler does not set the phase voltages
  // in time, they will go to zero.
  //
  // @tparam T Must be a callable type that takes no arguments and returns a
  // bool
  template <typename T>
  void run_control_loop(const T& update_handler) {
    while (requested_state_ == AXIS_STATE_UNDEFINED) {
      ...
      if (!checks_ok || !updates_ok || !watchdog_ok) {
        ...
        if (current_state_ != AXIS_STATE_IDLE) break;
      }
  ...
  constexpr uint32_t get_watchdog_reset() {
    return static_cast<uint32_t>(
        std::clamp<float>(config_.watchdog_timeout, 0,
                          UINT32_MAX / (current_meas_hz + 1)) *
        current_meas_hz);
  }
  ...
  bool step_dir_active_ =
      false;  // auto enabled after calibration, based on config.enable_step_dir
  ...
  std::array<AxisState, 10> task_chain_ = {AXIS_STATE_UNDEFINED};  // 状态
