...
// between any two interrupts (that is, enqueue_modulation_timings must be
// executed). If the control loop fails to do so, the next interrupt handler
// floats the phases. Once this happens, missed_control_deadline is set to true
// and the motor can be considered disarmed.
//
...
void Motor::update_current_controller_gains() {
  ...
  current_control_.p_gain =
      config_.current_control_bandwidth * config_.phase_inductance;
...
void Motor::DRV8301_setup() {
  ...
  // Solve for exact gain, then snap down to have equal or larger range as
  // requested or largest possible range otherwise
  constexpr float kMargin = 0.90f;
  constexpr float kTripMargin =
      1.0f;  // Trip level is at edge of linear range of amplifer
  constexpr float max_output_swing = 1.35f;  // [V] out of amplifier
  float max_unity_gain_current =
      kMargin * max_output_swing * hw_config_.shunt_conductance;  // [A]
  float requested_gain =
      max_unity_gain_current / config_.requested_current_range;  // [V/V]
  ...
  // We use lower_bound in reverse because it snaps up by default, we want to
  // snap down.
  auto gain_snap_down = std::lower_bound(
      gain_choices.crbegin(), gain_choices.crend(), requested_gain,
      [](std::pair<float, DRV8301_ShuntAmpGain_e> pair, float val) {
        return pair.first > val;
      });

  // If we snap to outside the array, clip to smallest val
  if (gain_snap_down == gain_choices.crend()) --gain_snap_down;
  ...
  current_control_.max_allowed_current =
      max_unity_gain_current * phase_current_rev_gain_;
  // Set trip level
  current_control_.overcurrent_trip_level =
      (kTripMargin / kMargin) * current_control_.max_allowed_current;
...
bool Motor::check_DRV_fault() {
  ...
  GPIO_PinState nFAULT_state = HAL_GPIO_ReadPin(gate_driver_config_.nFAULT_port,
                                                gate_driver_config_.nFAULT_pin);
  if (nFAULT_state == GPIO_PIN_RESET) {
    // Update DRV Fault Code
    gate_driver_exported_.drv_fault =
        (GateDriverIntf::DrvFault)DRV8301_getFaultType(&gate_driver_);
...
float Motor::effective_current_lim() {
  ...
  if (axis_->motor_.config_.motor_type == Motor::MOTOR_TYPE_GIMBAL) {
    current_lim = std::min(
        current_lim, 0.98f * one_by_sqrt3 *
                         vbus_voltage);  // gimbal motor is voltage control
  } else {
    current_lim = std::min(current_lim,
                           axis_->motor_.current_control_.max_allowed_current);
  }

  // Apply axis current limiters
  for (const CurrentLimiter* const limiter : axis_->current_limiters_) {
    current_lim =
        std::min(current_lim, limiter->get_current_limit(config_.current_lim));
  }
...
void Motor::log_timing(TimingLog_t log_idx) {
  static const uint16_t clocks_per_cnt =
      (uint16_t)((float)TIM_1_8_CLOCK_HZ / (float)TIM_APB1_CLOCK_HZ);
  uint16_t timing =
      clocks_per_cnt * htim13.Instance->CNT;  // TODO: Use a hw_config
...
bool Motor::measure_phase_resistance(float test_current, float max_voltage) {
  ...
  static const int num_test_cycles =
      (int)(3.0f / CURRENT_MEAS_PERIOD);  // Test runs for 3s
  ...
  if (axis_->error_ != Axis::ERROR_NONE) return false;
...
bool Motor::measure_phase_inductance(float voltage_low, float voltage_high) {
  ...
  // finite timestep. However, the discretisation in the current control loop
  // inverts the same discrepancy
  float dI_by_dt =
      (Ialphas[1] - Ialphas[0]) / (current_meas_period * (float)num_cycles);
...
bool Motor::run_calibration() {
  ...
  if (config_.motor_type == MOTOR_TYPE_HIGH_CURRENT ||
    ...
    if (!measure_phase_resistance(config_.calibration_current,
                                  R_calib_max_voltage))
      return false;
...
bool Motor::FOC_current(float Id_des, float Iq_des, float I_phase,
                        float pwm_phase) {
  // Syntactic sugar
  ...
  if (std::abs(current_meas_.phB) > ictrl.overcurrent_trip_level ||
      std::abs(current_meas_.phC) > ictrl.overcurrent_trip_level) {
  ...
  ictrl.Iq_measured +=
      ictrl.I_measured_report_filter_k * (Iq - ictrl.Iq_measured);
  ictrl.Id_measured +=
      ictrl.I_measured_report_filter_k * (Id - ictrl.Id_measured);
  ...
  if (mod_scalefactor < 1.0f) {
    ...
    ictrl.v_current_control_integral_d +=
        Ierr_d * (ictrl.i_gain * current_meas_period);
    ictrl.v_current_control_integral_q +=
        Ierr_q * (ictrl.i_gain * current_meas_period);
  }
...
bool Motor::update(float torque_setpoint, float phase, float phase_vel) {
  ...
  if (config_.motor_type == MOTOR_TYPE_ACIM) {
    current_setpoint = torque_setpoint / (config_.torque_constant *
                                          fmax(current_control_.acim_rotor_flux,
                                               config_.acim_gain_min_flux));
  } else {
    current_setpoint = torque_setpoint / config_.torque_constant;
  }
  ...
  if (config_.motor_type == MOTOR_TYPE_ACIM) {
    // Note that the effect of the current commands on the real currents is
    // actually 1.5 PWM cycles later However the rotor time constant is
    // (usually) so slow that it doesn't matter So we elect to write it as if
    // the effect is immediate, to have cleaner code

    if (config_.acim_autoflux_enable) {
      float abs_iq = fabsf(iq);
      float gain = abs_iq > id ? config_.acim_autoflux_attack_gain
                               : config_.acim_autoflux_decay_gain;
      id += gain * (abs_iq - id) * current_meas_period;
      id = std::clamp(id, config_.acim_autoflux_min_Id, ilim);
      current_control_.Id_setpoint = id;
    }

    // acim_rotor_flux is normalized to units of [A] tracking Id; rotor
    // inductance is unspecified
    float dflux_by_dt =
        config_.acim_slip_velocity * (id - current_control_.acim_rotor_flux);
    current_control_.acim_rotor_flux += dflux_by_dt * current_meas_period;
    float slip_velocity =
        config_.acim_slip_velocity * (iq / current_control_.acim_rotor_flux);
    // Check for issues with small denominator. Polarity of check to catch NaN
    // too
    bool acceptable_vel = fabsf(slip_velocity) <= 0.1f * (float)current_meas_hz;
    if (!acceptable_vel) slip_velocity = 0.0f;
    ...
    current_control_.async_phase_offset =
        wrap_pm_pi(current_control_.async_phase_offset);
  ...
  switch (config_.motor_type) {
    case MOTOR_TYPE_HIGH_CURRENT:
      return FOC_current(id, iq, phase, pwm_phase);
      break;
    case MOTOR_TYPE_ACIM:
      return FOC_current(id, iq, phase, pwm_phase);
      break;
    case MOTOR_TYPE_GIMBAL:
      return FOC_voltage(id, iq, pwm_phase);
      break;
    default:
