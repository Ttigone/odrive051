...
typedef struct {
  ...
  uint32_t
      min_stack_space_axis0;  // minimum remaining space since startup [Bytes]
...
struct BoardConfig_t {
  ...
  float dc_bus_undervoltage_trip_level =
      8.0f;  //<! [V] minimum voltage below which the motor stops operating
  float dc_bus_overvoltage_trip_level =
      1.07f * HW_VERSION_VOLTAGE;  //<! [V] maximum voltage above which the
                                   //motor stops operating.
                                   //<! This protects against cases in which the
                                   //power supply fails to dissipate
                                   //<! the brake power if the brake resistor is
                                   //disabled.
                                   //<! The default is 26V for the 24V board
                                   //version and 52V for the 48V board version.

  /**
   * If enabled, if the measured DC voltage exceeds
   * `dc_bus_overvoltage_ramp_start`, the ODrive will sink more power than usual
   * into the the brake resistor in an attempt to bring the voltage down again.
   *
  ...
  float dc_bus_overvoltage_ramp_start =
      1.07f * HW_VERSION_VOLTAGE;  //!< See `enable_dc_bus_overvoltage_ramp`.
                                   //!< Do not set this lower than your usual
                                   //!< vbus_voltage, unless you like fried
                                   //!< brake resistors.
  float dc_bus_overvoltage_ramp_end =
      1.07f * HW_VERSION_VOLTAGE;  //!< See `enable_dc_bus_overvoltage_ramp`.
                                   //!< Must be larger than
                                   //!< `dc_bus_overvoltage_ramp_start`,
                                   //!< otherwise the ramp feature is disabled.

  float dc_max_positive_current =
      INFINITY;  // Max current [A] the power supply can source
  float dc_max_negative_current =
      -0.000001f;  // Max current [A] the power supply can sink. You most likely
                   // want a non-positive value here. Set to -INFINITY to
                   // disable.
   ...
   *
   * For more information refer to Section 30.3.4 and Table 142 (the column with
   * f_PCLK = 42 MHz) in the STM datasheet:
...
#define DEFINE_ENUM_FLAG_OPERATORS(ENUMTYPE)                      \
  ...
  inline ENUMTYPE& operator|=(ENUMTYPE& a, ENUMTYPE b) {          \
    return reinterpret_cast<ENUMTYPE&>(                           \
        reinterpret_cast<std::underlying_type_t<ENUMTYPE>&>(a) |= \
        static_cast<std::underlying_type_t<ENUMTYPE>>(b));        \
  }                                                               \
  inline ENUMTYPE& operator&=(ENUMTYPE& a, ENUMTYPE b) {          \
    return reinterpret_cast<ENUMTYPE&>(                           \
...
class ODrive : public ODriveIntf {
 public:
  ...
  float get_adc_voltage(uint32_t gpio) override {
    return ::get_adc_voltage(get_gpio_port_by_pin(gpio),
                             get_gpio_pin_by_pin(gpio));
  }
  ...
  // If the first block starts with "0x00" and the second block starts with
  // "0xfe", we use the second block. This gives the user the chance to screw up
  // once. If none of the above is the case, we consider the OTP invalid
  // (otp_ptr will be NULL).
  const uint8_t* otp_ptr = (*(uint8_t*)FLASH_OTP_BASE == 0xfe)
                               ? (uint8_t*)FLASH_OTP_BASE
                           : (*(uint8_t*)FLASH_OTP_BASE != 0x00) ? NULL
                           : (*(uint8_t*)(FLASH_OTP_BASE + 0x10) != 0xfe)
                               ? NULL
...
#error "not implemented"
  ...
  const uint8_t fw_version_unreleased_ =
      ::fw_version_unreleased_;  // 0 for official releases, 1 otherwise

  bool& brake_resistor_armed_ =
      ::brake_resistor_armed;  // TODO: make this the actual variable
  bool& brake_resistor_saturated_ =
      ::brake_resistor_saturated;  // TODO: make this the actual variable
