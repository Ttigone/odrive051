...
class USBSender : public PacketSink {
 public:
  ...
  int process_packet(const uint8_t* buffer, size_t length) {
    ...
    if (length > USB_TX_DATA_SIZE) return -1;
    // wait for USB interface to become ready
    if (osSemaphoreWait(sem_usb_tx_, PROTOCOL_SERVER_TIMEOUT_MS) != osOK) {
      // If the host resets the device it might be that the TX-complete handler
      // is never called and the sem_usb_tx_ semaphore is never released. To
      // handle this we just override the TX buffer if this wait times out. The
      // implication is that the channel is no longer lossless.
      // TODO: handle endpoint reset properly
      usb_stats_.tx_overrun_cnt++;
    }
    // transmit packet
    uint8_t status = CDC_Transmit_FS(
        const_cast<uint8_t*>(buffer) /* casting this const away is safe
        because... well... it's not actually. Stupid STM. */
        ,
        length, endpoint_pair_);
    if (status != USBD_OK) {
 ...
 public:
  ...
  int process_bytes(const uint8_t* buffer, size_t length,
    ...
    while (length) {
      ...
      if (output_.process_packet(buffer, chunk) != 0) return -1;
      buffer += chunk;
      length -= chunk;
...
// This is used by the printf feature. Hence the above statics, and below
// seemingly random ptr (it's externed)
...
#if defined(USB_PROTOCOL_NATIVE)
          usb_channel.process_packet(CDC_interface.rx_buf,
                                     CDC_interface.rx_len);
#elif defined(USB_PROTOCOL_NATIVE_STREAM_BASED)
          usb_native_stream_input.process_bytes(CDC_interface.rx_buf,
                                                CDC_interface.rx_len, nullptr);
#endif
        }
        USBD_CDC_ReceivePacket(&hUsbDeviceFS,
                               CDC_interface.out_ep);  // Allow next packet
      }
...
#if defined(USB_PROTOCOL_NATIVE)
        usb_channel.process_packet(ODrive_interface.rx_buf,
                                   ODrive_interface.rx_len);
#elif defined(USB_PROTOCOL_NATIVE_STREAM_BASED)
        usb_native_stream_input.process_bytes(ODrive_interface.rx_buf,
                                              ODrive_interface.rx_len, nullptr);
#endif
        USBD_CDC_ReceivePacket(&hUsbDeviceFS,
                               ODrive_interface.out_ep);  // Allow next packet
      }
    }
  }
}
...
void usb_rx_process_packet(uint8_t* buf, uint32_t len, uint8_t endpoint_pair) {
  ...
  // We don't allow the next USB packet until the previous one has been
  // processed completely. Therefore it's safe to write to these vars directly
  // since we know previous processing is complete.
